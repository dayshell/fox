...
import { Copy, CheckCircle, Clock, ArrowRight, AlertCircle, MessageSquare, HelpCircle, X } from 'lucide-react';
...
function ExchangeContent() {
  ...
  // Step 1 = 0 (select pair - already done on main page)
  // Step 2 = 1 (enter wallet) - active when on payment page
  // Step 3 = 2 (make payment) - active when wallet entered
  // Step 4 = 3 (wait for coins) - active after payment confirmed
  const getCurrentHelpStep = () => {
    if (currentOrder?.status === 'completed') return 3;
    if (step === 2) return 3; // Waiting for coins
    // On step 1 (payment page), show steps 1 and 2 as completed, step 3 as active
    return 2; // Payment step is active
  };
  ...
  useEffect(() => {
    // Skip if we already have order data restored
  ...
  }, [searchParams, coins, currentOrder]);

  // Check if coin is fiat (no logo URL)
  const isFiat = (coin: Coin | null) => coin && !coin.logoUrl;

  // Find payment details - crypto wallet or bank details
  ...
  const hasPaymentMethod = isFromCoinFiat ? !!bankPayment : !!paymentWallet;
  ...
  const handleConfirmPayment = () => {
    ...
    // For fiat, also require cardholder name
    ...
    // Create order - include cardholder name for fiat
    ...
    // Send Telegram notification directly from localStorage
    try {
    ...
    // Update URL with order number for persistence
    router.replace(`/exchange?order=${orderNumber}`);
  };

  // Refresh order status periodically
  useEffect(() => {
    if (step === 2 && orderNumber && orders.length > 0) {
    ...
    <div className="min-h-screen py-12">
      <div className="max-w-3xl mx-auto px-4 relative">
        ...
        <div className="flex items-center justify-center gap-4 mb-8">
          ...
          <div className={`w-12 h-0.5 ${currentOrder?.status === 'completed' ? 'bg-orange-600' : 'bg-gray-800'}`} />
          <div className={`flex items-center gap-2 ${currentOrder?.status === 'completed' ? 'text-orange-400' : 'text-gray-600'}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${currentOrder?.status === 'completed' ? 'bg-orange-600' : 'bg-gray-800'}`}>
              3
            </div>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <span className="text-gray-400">{t('exchange.order')} #{orderNumber}</span>
        ...
        {step === 1 && (
          <>
            {/* Payment Details - Crypto or Bank */}
            <motion.div
            ...
            >
              {/* Help Bottom Sheet - slides up from bottom of this card */}
              <AnimatePresence>
                {showHelp && (
                  <motion.div
            ...
            {hasPaymentMethod ? (
              <motion.button
                ...
                className="w-full py-4 text-lg font-semibold rounded-xl bg-orange-600 hover:bg-orange-500 text-white"
